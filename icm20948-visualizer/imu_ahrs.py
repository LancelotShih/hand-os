...
Vendored from ``../icm20948-tool/imu_ahrs.py``.  The only change here is that
:meth:`OrientationTracker.process` also returns ``quat_abs`` (the filter's
absolute orientation, before the display reference is applied) and ``mbody``
(the calibrated magnetometer vector in the body frame) so that
:mod:`mag_localize` can reuse the same fused orientation and the same
...
class OrientationTracker:
    ...
    def process(self, accel_g, gyro_dps, mag_ut, now):
        ...
        return {
            ...
            "quat_abs": [round(v, 5) for v in q_abs],
            "mbody": None if mbody is None else [round(v, 4) for v in mbody],
